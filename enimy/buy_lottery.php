<?php
  ...
  require 'db_config.php'; // ไปเอาข้อมูลมาจาก db_config.php

  $userId   = $_SESSION['user_id'];
  $fullname = $_SESSION['fullname'];

  // ── AJAX buy ──────────────────────────────────────────────────────────────
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) { // ถ้าส่ง method post มา และ name="ajax" มี value
    header('Content-Type: application/json');
    $number = trim($_POST['number'] ?? ''); // ตัดช่องว่าง
    $units  = intval($_POST['units']  ?? 1); // แปลงเป็น int
    if (!preg_match('/^\d{6}$/', $number)) { //check $number /regex/ คล้ายกับ "hello"  ^ เริ่มต้นข้อความ  \d เลข 0-9   {6} มี6ตัวเท่านั้น   $ จบข้อความ
  ...
  $rows->execute([$userId]);
  $purchases = $rows->fetchAll(); // ดึงข้อมูลทั้งหมด
  $totalSpent = array_sum(array_column($purchases, 'price')); // column ต้องการจะดึง  เอา column มา sum
  $totalTickets = count($purchases); // นับว่ามีกี่ row
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
  ...
  <nav>
    ...
    <div class="nav-right">
      <div class="nav-user">👤 <?= htmlspecialchars($fullname) ?></div> <!-- กัน hacker ใส่ script -->
  ...
  <div class="container">
    ...
    <div class="stats-strip">
      ...
      <div class="stat-pill">
        <span class="s-icon">💰</span>
        <div>
          <div class="s-val" id="stat-spent">฿<?= number_format($totalSpent, 0) ?></div> <!-- จัด fomat ตัวเลข -->
      ...
      <div class="card">
        ...
        <div class="field">
          ...
          <input type="text" class="number-input" id="inp-number"
            ...
            oninput="this.value=this.value.replace(/\D/g,'');updatePreview()"> <!-- this.value ค่าของ input --> <!-- \D ไม่ใช่ตัวเลข --> <!-- g ค้นหาทั้งหมด -->
        </div>
  ...
  <script>
    ...
    async function buyLottery() {
      ...
      const res = await fetch('buy_lottery.php', {
        method: 'POST',
        body: fd
      });
